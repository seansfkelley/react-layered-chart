...
export function makeFakeBucketedData(endTime, xExtent, yExtent, bucketWidth = 1000 * 60 * 60 * 24) {
  ...
  while (startTime < endTime) {
    ...
    let value2;
    if (data.length) {
      value1 = data[data.length - 1].minValue + (Math.random() - 0.5) * yExtent * 0.1;
      value2 = data[data.length - 1].maxValue + (Math.random() - 0.5) * yExtent * 0.1;
    } else {
    ...
    data.push({
      ...
      minValue,
      maxValue,
      firstValue: Math.random() * (maxValue - minValue) + minValue,
      lastValue: Math.random() * (maxValue - minValue) + minValue
    });
