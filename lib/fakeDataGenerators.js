function makeFakeLineData(endTime, xExtent, yExtent) {
  const data = [];
  for (let i = 0; i < 100; ++i) {
...
function makeFakeEventData(endTime, xExtent, yExtent) {
  const data = [];
  for (let i = 0; i < 10; ++ i) {
    const start = endTime - Math.random() * xExtent;
    data.push({ timeSpan: { start, end: start + (1000 * 60 * 60 * (24 * Math.random())) }});
  }
  return data;
}

function makeFakeBucketedData(endTime, xExtent, yExtent, bucketWidth = 1000 * 60 * 60 * 24) {
  const data = [];
